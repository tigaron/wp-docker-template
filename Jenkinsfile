    ...
    stage('Deploy') {
      steps {
        script {
          ...
          def command = "aws ssm send-command --instance-ids ${instance} --region ap-southeast-3 --document-name AWS-RunShellScript --parameters command=\"cd /home/ssm-user && rm -rf wp-docker-template && git clone https://github.com/tigaron/wp-docker-template && cd wp-docker-template && ./start.sh\""
